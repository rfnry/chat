...
import { useChatUpload } from '../../src/utils/useChatUpload'
...
function Probe({
  ...
  onState: (s: ReturnType<typeof useChatUpload<{ url: string }>>) => void
}) {
  const state = useChatUpload(uploader)
  onState(state)
  return null
}
...
describe('useChatUpload', () => {
  it('tracks a successful upload through uploading → done', async () => {
    ...
    let latest: ReturnType<typeof useChatUpload<{ url: string }>> | null = null
    render(
      <Probe
        uploader={uploader}
        onState={(s) => {
