...
import { parseAssistantMentions } from '../../src/utils/parseMentions'
...
describe('parseAssistantMentions', () => {
  it('matches an assistant by exact name (case-insensitive)', () => {
    expect(parseAssistantMentions('hey @helper do X', [alice, helper]).recipients).toEqual(['a_helper'])
    expect(parseAssistantMentions('hey @HELPER do X', [alice, helper]).recipients).toEqual(['a_helper'])
  })

  it('ignores non-assistant identities', () => {
    expect(parseAssistantMentions('@alice @helper', [alice, helper]).recipients).toEqual(['a_helper'])
  })

  it('@here expands to all assistants in first-seen thread order', () => {
    expect(parseAssistantMentions('ping @here', [helper, codex, alice]).recipients).toEqual([
      'a_helper',
      'a_codex',
    ])
  })
  ...
  it('silently drops unknown mentions', () => {
    expect(parseAssistantMentions('@nobody @helper', [helper]).recipients).toEqual(['a_helper'])
  })

  it('dedupes repeated mentions but preserves first-seen order', () => {
    expect(parseAssistantMentions('@helper @codex @helper', [helper, codex]).recipients).toEqual([
      'a_helper',
      'a_codex',
    ])
  })
  ...
  it('returns empty recipients and spans for plain text', () => {
    const result = parseAssistantMentions('hello world', [helper])
    expect(result.recipients).toEqual([])
    expect(result.spans).toEqual([])
  })

  it('ignores system identities for @here', () => {
    expect(parseAssistantMentions('@here', [system, helper]).recipients).toEqual(['a_helper'])
  })

  it('merges @here with explicit mentions, still deduped', () => {
    expect(parseAssistantMentions('@helper @here', [helper, codex]).recipients).toEqual([
      'a_helper',
      'a_codex',
    ])
  })
  ...
  it('spans report the positional range of each named mention', () => {
    const result = parseAssistantMentions('hey @helper look', [helper])
  ...
  it('spans are not emitted for @here', () => {
    const result = parseAssistantMentions('ping @here', [helper, codex])
    expect(result.spans).toEqual([])
