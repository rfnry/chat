...
describe('useThreadActions().withRun', () => {
  beforeEach(() => {
  ...
  it('opens a Run on entry and closes it on return (eager mode)', async () => {
    ...
    const { result } = renderHook(() => useThreadActions('t_1'), { wrapper: harness(client) })

    await result.current.withRun(async () => {})
  ...
  it('multi-emit shares one Run id', async () => {
    ...
    await result.current.withRun(async (send) => {
  ...
  it('lazy: true skips run open if no emit happens', async () => {
    ...
    await result.current.withRun(async () => {}, { lazy: true })
  ...
  it('triggeredBy: Event extracts event.id for begin_run', async () => {
    ...
    await result.current.withRun(async () => {}, { triggeredBy: triggering })
  ...
  it('idempotencyKey passes through to begin_run', async () => {
    ...
    await result.current.withRun(async () => {}, { idempotencyKey: 'op_xyz' })
