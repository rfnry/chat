...
class MembersCache:
    def __init__(self, rest: RestTransport, *, ttl_seconds: float = 5.0) -> None:
        self._rest = rest
    ...
    async def get(self, thread_id: str) -> list[ThreadMember]:
        if not self.enabled:
            return await self._rest.list_members(thread_id)
    ...
    async def _fetch(self, thread_id: str) -> list[ThreadMember]:
        ...
        try:
            members = await self._rest.list_members(thread_id)
