...
def default_observability_sink() -> ObservabilitySink:
    if os.environ.get("RFNRY_OBSERVABILITY_FORMAT") == "json":
        return JsonlStderrSink()
    if os.environ.get("RFNRY_OBSERVABILITY_FORMAT") == "pretty":
...
def _format_pretty(record: ObservabilityRecord, *, color: bool) -> str:
    ...
    parts = [tag, f"{record.kind:<22}"]
    if record.scope_leaf:
        parts.append(f"scope={record.scope_leaf}")
    if record.thread_id:
