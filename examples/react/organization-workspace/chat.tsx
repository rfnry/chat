...
export function Chat({ workspace }: { workspace: Workspace }) {
  const { threadId, status, error, events, isWorking, text, setText, submit } =
    useChat(workspace)

  if (!threadId) return <p>Setting up {workspace.label} thread…</p>
    ...
    <>
      ...
      {isWorking && (
        <div style={{ color: '#888', margin: '8px 0' }}>
          {workspace.agentName} is working…
        </div>
      )}
      <form
        onSubmit={(e) => {
